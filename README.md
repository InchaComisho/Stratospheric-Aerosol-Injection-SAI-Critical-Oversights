...
```mermaid
flowchart TD
    A[Warming] --> B[Soil drying, forest loss, wetland loss]
    B --> C[Bare land, desertification, urban drying]
    C --> D[Dust, pollen, smoke, and PM-like particles become easier to lift]
    D --> E[Localized rainfall and weaker atmospheric cleaning]
    E --> F[Particles are not sufficiently washed out]
    F --> G[Deposited particles resuspend from dry surfaces]
    G --> H[Atmospheric particle loading remains chronically elevated]
    H --> I[Effects on sunlight, infrared radiation, clouds, and rainfall]
    I --> J[Water circulation and natural cooling feedbacks weaken further]
    J --> A
```
