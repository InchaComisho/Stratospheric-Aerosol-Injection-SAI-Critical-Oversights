...
```mermaid
flowchart TD
    A[Global Warming] --> B[Soil Drying]
    B --> C[Forest and Wetland Loss]
    C --> D[Bare Land and Urban Dry Surfaces]
    D --> E[More Dust, Pollen, Smoke, PM-like Particles]
    E --> F[Higher Atmospheric Particle Load]
    F --> G[Cloud, Rainfall, Radiation, and Health Effects]
    G --> H[Weaker Rain Cleaning and Wet Deposition]
    H --> I[Particles Not Fully Removed]
    I --> J[Particles Settle on Dry Surfaces]
    J --> K[Resuspension by Wind, Vehicles, Turbulence, and Heating]
    K --> F
    H --> L[Weaker Water Cycle]
    L --> A
```
